...
export const Tests = (props: {teacherId?: string, setActive: (value: string) => void, currentUser: any}) => {
    ...
    useEffect(() => {
        ...
        setCanChange(props.currentUser?.id === props.teacherId ?? false);
    }, [])
    const [canChange, setCanChange] = useState(props.currentUser?.id === props.teacherId ?? false);
    ...
    return <div className='material' >
        {tests ? tests.map((e) => <div className={'test-element'}>
            ...
            : <div> Добавьте первый тест! </div>}
        {canChange && <Button type={'submit'} onClick={() => navigate(`/profile/test?teacherId=${props.teacherId}&testId=new`)}>Добавить тест</Button>}
    </div>
}