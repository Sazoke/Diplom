...
public class SchoolAreaService : ISchoolAreaService
{
    ...
    public SchoolArea GetById(long id) => 
        _repository.GetById(id, q => q.Include(s => s.Tags));

    public IEnumerable<SchoolArea> GetAll() =>
        _repository.GetAll(q => q.Include(s => s.Tags));

    public async Task AddOrUpdate(SchoolAreaDto schoolAreaDto)
    {
        var schoolArea = schoolAreaDto.Id is null ? new SchoolArea() : GetById(schoolAreaDto.Id.Value);
